...
| المعمارية المدعومة | كلا المعماريتين صراحة: **64-بت (x64)** و **32-بت (ia32)** |
| قاعدة البيانات | SQLite عبر `better-sqlite3@8.7.0` (إضافة C++ أصلية، نمط WAL) |
| الواجهة | HTML/CSS/JS خام بدون إطار عمل (لا React/Vue) |
| نقطة الدخول | `src/main/main.js` |
| التخزين | ملف `leave_management.db` + مجلد `EmployeeDocuments/` على نظام الملفات المحلي |

**لماذا هذا الاختيار:** Electron 22.3.27 هو آخر إصدار رسمي من Electron يدعم Windows 7/8/8.1 (حيث يتطلب Electron 23+ محرك Chromium 110 الذي أسقط دعم هذه الأنظمة). الحفاظ على هذه الحزمة يضمن تشغيل البرنامج في كافة الدوائر الحكومية التي لا تزال تستخدم أجهزة Windows 7 القديمة والحديثة على حد سواء.
...
flowchart TD
    ...
    B["Preload Bridge
    (src/main/preload.js)
    contextBridge + قائمة بيضاء صارمة VALID_CHANNELS (47 قناة)"]
    C["IPC Handlers
    (src/main/ipc/*.js)
    طبقة تفويض رقيقة فقط (بعد N3) — استقبال الطلب وتفويضه للخدمة وتغليف الاستجابة"]
    D["Business Services
    (src/main/services/*.js)
    منطق الأعمال، المعاملات الذرية، قواعد التحقق
    (LeaveService / EmployeeService / DocumentService / ReportService / AuditService)"]
    E["Data Access
    (src/main/database.js)
    بعد تنظيف N1: التهيئة، الترحيلات (001-016)، النسخ الاحتياطي والاستعادة فقط"]
    F[("SQLite (WAL)
    + EmployeeDocuments/
    على نظام الملفات")]
...
الملف الوحيد المسموح له بالوصول لكل من `electron` (`contextBridge`, `ipcRenderer`) وبيئة المتصفح (`window`) في آن واحد. يبني كائن `window.api` عبر `contextBridge.exposeInMainWorld('api', {...})`، مقسّماً حسب النطاق الوظيفي: `employees`، `leaveTypes`، `leaveBalances`، `leaves`، `leave`، `employee`، `report`، `audit`، `system`، `document`.

**آلية الحماية المحورية:** كل استدعاء يمر عبر دالة `invoke(channel, ...args)` داخلية تتحقق أولاً أن `channel` موجود ضمن `Set` باسم `VALID_CHANNELS` (يحتوي على **47 قناة معتمدة** بعد إزالة القنوات القديمة في N1) قبل تمريره لـ `ipcRenderer.invoke` — أي قناة غير مُدرَجة تُرفَض فوراً بـ `Promise.reject` دون أن تصل للعملية الرئيسية إطلاقاً.

**🛡️ نمط IPC Channel Guard (`tests/ipcChannelGuard.test.js`):**
وحدة اختبار آلية مدمجة تفحص مطابقة الـ 47 قناة IPC بدقة متناهية:
...
الملفات الستة المسؤولة عن توجيه القنوات:
* `employeeHandlers.js`: تفويض حركات الموظفين (إضافة، بحث، تعديل، تفعيل/تجميد، نقل، استعادة) إلى `EmployeeService`.
* `leaveHandlers.js`: تفويض حركات الإجازات (تسجيل مرضي/اعتيادي، استعلام أرصدة، إجازات نشطة، حذف، تعديل) مباشرة وبشكل رقيق إلى `LeaveService`.
* `reportHandlers.js`: تفويض تقارير وتصديرات Excel إلى `ReportService`.
...
| الخدمة | المسؤولية |
|---|---|
| `LeaveService.js` | محرك احتساب الإجازات: `calculateRegularLeaveBalance`، `processSickLeave`، `processRegularLeave`، `checkLeaveOverlap`، `getActiveLeavesForToday`، `updateLeave`، `deleteLeave` |
| `EmployeeService.js` | دورة حياة الموظف، النقل الخارجي وإلغاء النقل، التحقق من التكرار والتدقيق |
| `ReportService.js` | بناء تقارير Excel بتنسيق RTL احترافي عبر `exceljs` |
...
1. **التحقق الاستباقي المسبق (Pre-flight Validation):** التحقق من صحة التواريخ، كون الموظف نشطاً، حساب عدد الأيام الصافي، وسلامة المدخلات.
2. **فحص التداخل الزمني المسبق (`checkLeaveOverlap`):** منع تسجيل أي إجازة تتداخل مع إجازة سابقة لنفس الموظف قبل الشروع بالعملية.
...
تتضمن وحدة اختبارات متقدمة مبنية خصيصاً على معمارية Electron Node (ABI-110) عبر جسر داخلي شفاف، تختبر 24 حالة دقيقة تشمل:
- احتساب الإجازة ليوم واحد وخصم الرصيد.
- رفض الإجازات ذات التواريخ المقلوبة (`EndDate < StartDate`).
- الاحتساب الدقيق عبر السنوات الكبيسة (29 شباط).
- منع التداخل الزمني بصرامة لكافة الحالات.
...
| **N4** | اختبارات آلية لمحرك الإجازات وتنظيف رسائل السجل | `tests/leaveService.test.js` و `LoggerService` | **تم الإنجاز بالكامل (Resolved):** إنشاء 24 اختبار وحدة تغطي كافة الحالات والحدود، وتنظيف كافة `console.log` في المشروع. |
...
5. **تشغيل الاختبارات الآلية (`npm test`) إلزامياً قبل أي دمج أو نشر** — للتحقق من سلامة قنوات IPC (47 قناة) واختبارات محرك الإجازات والشرائح الثلاث (24 اختباراً).
...
*آخر تحديث لهذه الوثيقة: 9 أيلول 2026، بعد اكتمال تنفيذ المراحل الهندسية الأربع (N1–N4).*
