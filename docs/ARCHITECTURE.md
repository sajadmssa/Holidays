...
| الخدمة | المسؤولية |
|---|---|
| `LeaveService.js` | محرك احتساب الإجازات: `calculateRegularLeaveBalance` (معتمد توقيت بغداد الرسمي)، `processSickLeave`، `processRegularLeave`، `checkLeaveOverlap`، `getActiveLeavesForToday` (سارية وقادمة)، `getExpiredLeaves` (فترات مرنة)، `updateLeave`، `deleteLeave` |
| `EmployeeService.js` | دورة حياة الموظف، إدارة التسلسل التراكمي الثابت عبر `AppCounters`، النقل الخارجي وإلغاء النقل، والتحقق من التكرار وتفرد رقم الإضبارة |
...
| `ReportService.js` | بناء تقارير Excel بتنسيق RTL احترافي عبر `exceljs` (كشف الإجازات الحالية والقادمة بـ 16 عموداً، كشف الإجازات المنتهية بـ 17 عموداً، تقارير الأرصدة والتراكم، سجل الموظفين، والمستندات) مع التنسيق الرقمي الموحد |
...
| `LoggerService.js` | تسجيل مركزي موحّد للأخطاء والتحذيرات إلى ملف السجل مع تنقيح البيانات الحساسة PII |
...
3. **التحقق من الرصيد والاحتساب:**
   * **في الإجازة الاعتيادية (`processRegularLeave`):** فحص كفاية الرصيد استناداً إلى تاريخ اليوم المستخرج بتوقيت بغداد (`Asia/Baghdad`) عبر `Intl.DateTimeFormat` لمنع انزياح اليوم الواحد بين 00:00 و 03:00 فجراً؛ وفي حال كان الرصيد غير كافٍ ولم يُصرّح المستخدم بتسجيل رصيد سالب/عجز، يُرفض الطلب فوراً برسالة دقيقة توضح الرصيد المتاح والأيام المطلوبة؛ وإن صُرّح، يتم خصم الرصيد واحتساب العجز بدقة.
...
تتضمن وحدة اختبارات متقدمة مبنية خصيصاً على معمارية Electron Node (ABI-110) عبر جسر داخلي شفاف، تختبر 29 جناح فحص شاملاً (تضم 214 فحصاً مؤكداً) تشمل:
...
- احتساب أيام الإجازة عبر حدود الأشهر والسنوات الميلادية بدقة وتوقيت بغداد.
...
2. **إدارة الترحيلات:** تشغيل ملفات `src/main/migrations/` بالترتيب من `001` إلى `024` داخل معاملات ذرية مع تسجيل أسماء الترحيلات في جدول `_Migrations` لمنع تكرارها.
3. **النسخ الاحتياطي والاستعادة المجمعة:** إنشاء حزم `.hbak` والتحقق من سلامتها (`validateDatabaseBackup`)، وأخذ نسخة أمان فورية (`createSafetyBackup`) قبل أي عملية استعادة مع دعم التراجع الطارئ (`_emergencyRollback`) وحماية Zip Slip عبر `pathValidator.js`.
4. **توفير مرجع قاعدة البيانات والإغلاق الآمن:** إتاحة الدالة `getDb()` للوصول الآمن لمرجع قاعدة البيانات، وتحصين `db.close()` بكتلة `try/finally` لضمان تحرير الأقفال وتصفير المؤشر دائماً عند أي إغلاق طارئ أو خروج من التطبيق.

### 3.6 دورة حياة التطبيق وشبكة أمان المعالجة — `src/main/main.js` (ADR-031)

تم تعزيز نقطة الدخول الرئيسية بشبكة أمان متعددة الحصون تضمن عدم تلف البيانات عند الأخطاء الحرجة:
1. **معالجات الأمان العامة (`uncaughtException` / `unhandledRejection`):**
   - اعتراض أي استثناء غير مُلتقط أو وعد مرفوض على مستوى العملية.
   - تسجيل الخطأ تفصيلياً عبر `LoggerService.error`.
   - عرض نافذة خطأ للمستخدم باللغة العربية تشرح سبب الإيقاف الوقائي لحماية سلامة البيانات.
   - استدعاء `db.close()` لإغلاق الاتصال بأمان وتفريغ أقفال WAL و SHM قبل تنفيذ `app.exit(1)`.
2. **حماية حلقة انهيار الواجهة (`render-process-gone`):**
   - رصد انهيار واجهة المستخدم وعزل سبب الانهيار.
   - تقييد محاولات إعادة التحميل التلقائية بحد أقصى محاولة واحدة خلال نافذة 60 ثانية متداولة عبر `rendererCrashTimestamps`، وعرض حوار تحذيري في حال تكرار الانهيار بدلاً من الدخول في حلقة لا نهائية.
3. **عزل خطوات بدء التشغيل الثلاث:**
   - عزل تسجيل قنوات IPC (`registerIpcHandlers`)، وإنشاء النافذة (`createWindow`)، وبدء المجدول اليومي (`startNotificationScheduler`) في كتل `try/catch` مستقلة لتقديم تشخيص دقيق، ومنع فشل المجدول غير الحرج من تعطيل فتح التطبيق.

---

## 4. خريطة قنوات IPC الكاملة (الحالة الفعلية)

### 4.1 القنوات المعتمدة والنشطة (54 قناة)

| المجموعة | القنوات (54 قناة معتمدة) |
|---|---|
...
| `leaveType:*` (إدارة أنواع الإجازات) | `leaveType:getAll`، `leaveType:add`، `leaveType:update`، `leaveType:delete` (4 قنوات) |
...
> **إجمالي القنوات المعتمدة حالياً في `VALID_CHANNELS` ومطابقة تماماً للمعالجات:** 54 قناة بدقة 100%.
