...
if (!gotTheLock) {
  ...
  // ──────────────────────────────────────────────────────────────
  //  Global Process Safety Net / شبكة الأمان العامة للعملية الرئيسية
  // ──────────────────────────────────────────────────────────────
  process.on('uncaughtException', (err) => {
    LoggerService.error('Process', 'Uncaught Exception detected', err);
    try {
      dialog.showErrorBox('خطأ غير متوقع في النظام', 'حدث خطأ غير متوقع أدى لإيقاف البرنامج لحماية سلامة البيانات. يرجى إعادة تشغيل البرنامج، وفي حال تكرار المشكلة يرجى التواصل مع الدعم الفني.');
    } catch (_) { /* dialog قد لا يكون متاحاً في حالات نادرة جداً */ }
    try {
      db.close();
    } catch (_) { /* تجاهل فشل الإغلاق، الأولوية للخروج الآمن */ }
    app.exit(1);
  });

  process.on('unhandledRejection', (reason) => {
    LoggerService.error('Process', 'Unhandled Promise Rejection detected', reason);
    try {
      dialog.showErrorBox('خطأ غير متوقع في النظام', 'حدث خطأ غير متوقع أدى لإيقاف البرنامج لحماية سلامة البيانات. يرجى إعادة تشغيل البرنامج، وفي حال تكرار المشكلة يرجى التواصل مع الدعم الفني.');
    } catch (_) {}
    try {
